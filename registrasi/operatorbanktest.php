                ...
                #{
                   ...
                   #print_r($valdata).'<br>';
					...
					#echo "PANJANG=".strlen($data[hp]);exit();
						...
						$sql_cek_email="SELECT EMAIL FROM calonmahasiswatest WHERE EMAIL='{$data['email']}' AND PRODI1='$idprodi1'";
		...
		if ( 0 < sqlnumrows( $h_biaya ) )
			...
			$biaya_per_prodi=0;
							...
							if ( 0 < sqlaffectedrows( $koneksi ) )
							{
								...
								$dmail['subject'] = str_replace( "[NAMACALONMAHASISWA]", "{$namacalonmhs}", $dmail[subject] );
...
# echo $q;exit();
		...
		#echo $kodemenu;
   	 	#cekhaktulis($kodemenu,"bank");
    		printjudulmenu( "Test Tambah Data Calon Mahasiswa (semua data harus terisi dengan benar)" );
    		printmesg($errmesg);
