...
if ( !( ereg_sikad( "1", $caraisidh ) || ereg_sikad( "3", $caraisidh ) ) )
{
    exit( );
}
...
if ( $iduser == "superadmin" )
{
    ...
    Header( "Location: indexb.php?err=admin&iduser={$iduser}" );
}
$w = getdate( time( ) );
...
if ( trim( $aksiterlambat ) == "Isi" )
{
    ...
    {
        $query = "INSERT INTO presensi VALUES\r\n\t\t\t('{$iduser}',NOW(),NOW(),NULL,'TS',NULL,'{$ket}',NULL,\r\n\t\t\t'".getasal( )."',NULL,{$idlokasikantor},'{$shift}',0,'',0,NULL,NULL,0,NULL,NULL,NULL)";
        mysqli_query($koneksi,$query);
        if ( sqlaffectedrows( $koneksi ) == 1 )
        {
            if ( cekisidatang( "{$iduser}", $pindahhari ) == true )
            {
                mysqli_close($koneksi );
                Header( "Location: indexb.php?err=datangtelat&iduser={$idusr}" );
            }
            else
            {
                mysqli_close($koneksi );
                Header( "Location: indexb.php?err=koneksi&iduser={$idusr}" );
            }
        }
        else
        {
            mysqli_close($koneksi );
            Header( "Location: indexb.php?err=keterangangagal&iduser={$idusr}" );
        }
    }
    else
    {
        ...
        if ( sqlaffectedrows( $koneksi ) == 1 )
        {
            ...
            {
                mysqli_close($koneksi );
                Header( "Location: indexb.php?err=datangbatastoleransi&iduser={$idusr}" );
            }
            else
            {
                mysqli_close($koneksi );
...
if ( trim( $aksipulangcepat ) == "Isi" )
{
    ...
    if ( sqlaffectedrows( $koneksi ) == 1 )
    {
        ...
        {
            mysqli_close($koneksi );
            Header( "Location: indexb.php?err=pulangcepat&alasan={$alasan}&iduser={$idusr}" );
        }
        else
        {
            mysqli_close($koneksi );
...
if ( $aksi == "isipresensi" )
{
    ...
    {
        mysqli_close($koneksi );
        Header( "Location: indexb.php?err=noid" );
    }
    else if ( $pass == "" )
    {
        mysqli_close($koneksi );
        Header( "Location: indexb.php?err=nopass" );
    }
    else
    {
        ...
        $wnow = getdate( time( ) );
        $query = "SELECT NAMA,SHIFT FROM user WHERE ID='{$iduser}' AND PASSWORD='{$pass}' AND ID!='superadmin' AND STATUSKERJA=0";
        $hasil = mysqli_query($koneksi,$query);
        if ( sqlnumrows( $hasil ) == 0 )
        {
            mysqli_close($koneksi );
            Header( "Location: indexb.php?err=nouser&iduser={$iduser}" );
        }
        else
        {
            ...
            {
                mysqli_close($koneksi );
                Header( "Location: indexb.php?err=liburs&iduser={$idusr}" );
            }
            else
            {
                if ( $w[wday] == 5 )
                ...
                if ( cekisidatang( "{$iduser}", $pindahhari ) == false )
                {
                    if ( ereg_sikad( "1", $caraisidhdatang ) )
                    {
                        ...
                        {
                            mysqli_close($koneksi );
                            Header( "Location:indexb.php?err=belumisijadwal&iduser={$idusr}&pindahhari={$pindahhari}" );
                            exit( );
                        }
                    }
                    ...
                    if ( sqlnumrows( $hasil ) == 0 )
                    {
                        ...
                        {
                            $query = "INSERT INTO presensi VALUES\r\n\t\t\t\t\t\t\t\t\t('{$iduser}',NOW(),NOW(),NULL,'N',NULL,NULL,NULL,\r\n\t\t\t\t\t\t\t\t\t'".getasal( )."',NULL,{$idlokasikantor},\r\n\t\t\t\t\t\t\t\t\t'{$datauser['SHIFT']}',0,'',0,NULL,NULL,0,NULL,NULL,NULL)";
                            mysqli_query($koneksi,$query);
                            if ( 0 < sqlaffectedrows( $koneksi ) )
                            {
                                ...
                                {
                                    mysqli_close($koneksi );
                                    Header( "Location: indexb.php?err=datangtepatwaktu&iduser={$idusr}&jam={$jam}" );
                                }
                                else
                                {
                                    mysqli_close($koneksi );
                        ...
                        else
                        {
                            ...
                            {
                                $toleran = "awal";
                                $query = "INSERT INTO presensi VALUES\r\n\t\t\t\t\t\t\t\t\t('{$iduser}',NOW(),NOW(),NULL,'T',NULL,NULL,NULL,\r\n\t\t\t\t\t\t\t\t\t'".getasal( )."',NULL,{$idlokasikantor},\r\n\t\t\t\t\t\t\t\t\t'{$shift}',0,'',0,NULL,NULL,0,NULL,NULL,NULL)";
                                mysqli_query($koneksi,$query);
                                if ( 0 < sqlaffectedrows( $koneksi ) )
                                {
                                    if ( cekisidatang( "{$iduser}", $pindahhari ) == true )
                            ...
                            else
                            {
                                ...
                                {
                                    $toleran = "akhir";
                                    $query = "INSERT INTO presensi VALUES\r\n\t\t\t\t\t\t\t\t\t('{$iduser}',NOW(),NOW(),NULL,'T',NULL,\r\n\t\t\t\t\t\t\t\t\tNULL,NULL,'".getasal( )."',NULL,{$idlokasikantor},\r\n\t\t\t\t\t\t\t\t\t'{$shift}',0,'',0,NULL,NULL,0,NULL,NULL,NULL)";
                                    mysqli_query($koneksi,$query);
                                    if ( 0 < sqlaffectedrows( $koneksi ) )
                                    {
                                        if ( cekisidatang( "{$iduser}", $pindahhari ) == true )
                                ...
                                else
                                {
                                    ...
                                    {
                                        $status = "telatsekali";
                                        $query = "INSERT INTO presensi VALUES\r\n\t\t\t\t\t\t\t\t\t('{$iduser}',NOW(),NOW(),NULL,'TS',NULL,'{$ket}',\r\n\t\t\t\t\t\t\t\t\tNULL,'".getasal( )."',NULL,{$idlokasikantor},\r\n\t\t\t\t\t\t\t\t\t'{$shift}',0,'',0,NULL,NULL,0,NULL,NULL,NULL)";
                                        mysqli_query($koneksi,$query);
                                        if ( sqlaffectedrows( $koneksi ) == 1 )
                                        {
                                            if ( cekisidatang( "{$iduser}", $pindahhari ) == true )
                ...
                else
                {
                    ...
                    if ( ereg_sikad( "1", $caraisidhpulang ) )
                    {
                        ...
                        {
                            mysqli_close($koneksi );
                            Header( "Location:indexb.php?err=belumisikegiatan&iduser={$idusr}&pindahhari={$pindahhari}" );
                            exit( );
                        }
                    }
                    ...
                    if ( sqlnumrows( $hasil ) == 1 && $data[TGL] == $data[TGLNOW] )
                    {
                        ...
                        {
                            mysqli_close($koneksi );
                            Header( "Location: indexb.php?err=sudahabsenpulang&iduser={$idusr}" );
                        }
                        else
                        {
                            if ( $wpulang <= $now || $datauser[SHIFT] == 255 )
                            {
                                if ( $data[STATUSDATANG] == "N" || $datauser[SHIFT] == 255 )
                                {
                                    ...
                                    if ( 0 < sqlaffectedrows( $koneksi ) )
                                    {
                                        ...
                                        {
                                            mysqli_close($koneksi );
                                            Header( "Location: indexb.php?err=pulangtepatwaktu&iduser={$idusr}" );
                                        }
                                        else
                                        {
                                            mysqli_close($koneksi );
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        mysqli_close($koneksi );
                                        Header( "Location: indexb.php?err=kompensasi&iduser={$idusr}&jam={$jam}&mnt={$mnt}&dtk={$dtk}" );
                                    }
                                }
                            }
                            else
                            {
                                if ( $now < $wpulang )
                                {
                                    $query = "UPDATE presensi SET JAMPULANG=NOW(), \r\n\t\t\t\t\t\t\t\tSTATUSPULANG='I',ASALPULANG='".getasal( )."'\r\n\t\t\t\t\t\t\t\t\tWHERE IDUSER='{$iduser}' AND TANGGAL=DATE_ADD('{$wnow['year']}-{$wnow['mon']}-{$wnow['mday']}',\r\n\t\t\t\t\tINTERVAL {$hpindah} DAY)";
                                    mysqli_query($koneksi,$query);
                                    if ( 0 < sqlaffectedrows( $koneksi ) )
                                    {
                                        if ( cekisipulang( "{$iduser}", $pindahhari ) == true )
                    ...
                    {
                        mysqli_close($koneksi );
                        Header( "Location: indexb.php?err=belumabsendatang&iduser={$idusr}" );
                    }
                }
            }
        }
