...
// paging
	...
	$query = "SELECT COUNT(*) as num FROM dosen where 1=1 ";
	if($type=='tab'){
		$query.= 'and dosen.nama like \''.$pnama.'%\'';
	}elseif($type=='cari' ){
		if($pnama!=""){ 
			$query.= 'and dosen.nama like \'%'.$pnama.'%\'';
		}
		if($idprodi!=""){
			$query.='and IDDEPARTEMEN=\''.$idprodi.'\'';
		}
	}elseif($type=='semua'){
		$query.="";
	}
...
$sqltext = "Select dosen.ID as username, dosen.NAMA as nama_pengguna,IDDEPARTEMEN AS IDPRODI \r\n\t \r\n\tfrom dosen  ";
$sqltext = $sqltext." where \r\n\t\t\t 1=1 {$qf}  ";
if ( $tab == 1 )
{
    $sqltext = $sqltext."AND dosen.NAMA like '{$pnama}%' ";
}
else
{
    if ( $idprodi != "" )
    ...
    $sqltext = $sqltext."AND dosen.NAMA like '%{$pnama}%' {$qf2} ";
}
if ( $porder == "U" )
{
    $sqltext = $sqltext."order by dosen.NAMA";
}
else if ( $porder == "P" )
{
    $sqltext = $sqltext."order by dosen.IDDEPARTEMEN";
}
else
{
    $sqltext = $sqltext."order by dosen.ID";
}
...
while ( $i < 26 )
{
    echo "\r\n             \t<li><a href='listdosen.php?tab=1&pfld=".$pfld."&pfltr=".$pfltr."&type=tab&pnama=".chr( $i + 65 )."'>".chr( $i + 65 )."\r\n             \t</a></li>";
    ++$i;
}
...
echo "<s";
echo "pan class=\"smallspan\">ID PRODI :</span>\r\n         ";
echo "<s";
...
window.location.href='listdosen.php?pfld='+'{$pfld}'+'&type=cari&pfltr='+'{$pfltr}'+'&pnama='+katakunci+'&idprodi='+idprodi;
}
</script>";

echo "    \r\n    </form>\r\n    \r\n    ";
echo "<a name='semua' class='showall' href='listdosen.php?pfld=".$pfld."&pfltr=".$pfltr."&type=semua'>Tampilkan Semua</a>";
echo "    \r\n    </div>\r\n    \r\n    \r\n    \r\n\r\n\r\n\r\n";
...
echo "<td class=judul><a href='listdosen.php?$uri&porder=N'>ID /  NIDN</a></td>\n";
echo "<td class=judul><a href='listdosen.php?$uri&porder=U'>Nama</a></td>\n";
echo "<td class=judul><a href='listdosen.php?$uri&porder=P'>Jurusan/Prodi</a></td>\n";
echo "</tr></thead>\n";
$i = 0;
