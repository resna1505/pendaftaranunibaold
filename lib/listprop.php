...
// paging
	...
	$query = "SELECT COUNT(*) as num FROM tbpro where 1=1 ";
	if($type=='tab'){
		$query.= 'and tbpro.NMPROTBPRO like \''.$pnama.'%\'';
	}elseif($type=='cari' ){
		if($pnama!=""){
			$query.= "and tbpro.NMPROTBPRO like '%$pnama%' ";
		}
...
$sqltext = "Select  tbpro.KDPROTBPRO as username, tbpro.NMPROTBPRO as nama_pengguna ,NMKABTBPRO as kab,KDKABTBPRO as kodekab\r\n\t \r\n\tfrom tbpro  ";
...
while ( $i < 26 )
{
    echo "\r\n             \t<li><a href='listprop.php?tab=1&pfld=".$pfld."&pfltr=".$pfltr."&type=tab&pnama=".chr( $i + 65 )."'>".chr( $i + 65 )."\r\n         \t</a></li>";
    ++$i;
}
...
// echo "<input type=button class=login  value='Cari!' onClick=window.open('listprop.php?pfld='+'{$pfld}'+'&pfltr='+'{$pfltr}'+'&pnama='+katakunci.value,'_self')>";
echo "    \r\n    </form>\r\n    \r\n    ";
echo "<a class='showall' href='listprop.php?pfld=".$pfld."&type=semua&pfltr=".$pfltr."'>Tampilkan Semua</a>";
echo "    \r\n    </div>\r\n\r\n";
if($satu==1){
}else{
...
//echo "<td class=judul2 width='10'><a href='listprop.php?tab={$tab}&pfld=".$pfld."&pfltr=".$pfltr."&pnama={$pnama}&porder=N'>Kode</a></td>\n";
echo "<td class=judul2 width='10'><a href='listprop.php?$uri&porder=N'>Kode</a></td>\n";
echo "<td class=judul2 ><a href='listprop.php?$uri&porder=U'>Nama Prop</a></td>\n";
echo "<td class=judul2 ><a href='listprop.php?$uri&porder=V'>Nama Kab</a></td>\n";
echo "</tr></thead>\n";
$i = 0;
