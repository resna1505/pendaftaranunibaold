...
// paging
	...
	$query = "SELECT COUNT(*) as num FROM mspst where 1=1 ";
	if($type=='tab'){
		$query.= 'and mspst.NMPSTMSPST like \''.$pnama.'%\'';
	}elseif($type=='cari' ){
		if($pnama!=""){
			$query.= "and mspst.NMPSTMSPST like '%$pnama%' ";
		}
...
$sqltext = "Select mspst.KDPSTMSPST as username, mspst.NMPSTMSPST as nama_pengguna ,\r\nKDJENMSPST AS JENJANG\r\n\t \r\n\tfrom mspst  ";
$sqltext = $sqltext." where \r\n\t\t\t 1=1 {$qf}  ";
if ( $tab == 1 )
{
    $sqltext = $sqltext."AND mspst.NMPSTMSPST like '{$pnama}%' ";
}
else
{
    $sqltext = $sqltext."AND mspst.NMPSTMSPST like '%{$pnama}%' ";
...
if ( $porder == "U" )
{
    $sqltext = $sqltext."order by mspst.NMPSTMSPST";
}
else if ( $porder == "J" )
{
    $sqltext = $sqltext."order by mspst.KDJENMSPST";
}
else
{
    $sqltext = $sqltext."order by mspst.KDPSTMSPST";
}
...
while ( $i < 26 )
{
    echo "\r\n             \t<li><a href='listprodipt.php?tab=1&pfld=".$pfld."&pfltr=".$pfltr."&type=tab&pnama=".chr( $i + 65 )."'>".chr( $i + 65 )."\r\n         \t</a></li>";
    ++$i;
}
...
//echo "<input type=button class=login  value='Cari!' onClick=window.open('listprodipt.php?pfld='+'{$pfld}'+'&pfltr='+'{$pfltr}'+'&pnama='+katakunci.value,'_self')>\r\n";
echo "    \r\n    </form>\r\n    \r\n    ";
echo "<a class='showall' href='listprodipt.php?pfld=".$pfld."&type=semua&pfltr=".$pfltr."'>Tampilkan Semua</a>";
echo "    \r\n    </div>\r\n    \r\n    \r\n\r\n";
if($satu==1){
}else{
...
echo "<td class=judul2 width='10'><a href='listprodipt.php?$uri&porder=N'>Kode</a></td>\n";
echo "<td class=judul2 ><a href='listprodipt.php?$uri&porder=J'>Jenjang</a></td>\n";
echo "<td class=judul2 ><a href='listprodipt.php?$uri&porder=U'>Nama</a></td>\n";
echo "</tr></thead>\n";

// echo "<table width='100%' {$borderdata} class=data>\n";
// echo "<thead><tr class='juduldata' align=center>\n";
// echo "<td class=judul2 width='10'><a href='listprodipt.php?tab={$tab}&pfld=".$pfld."&pfltr=".$pfltr."&pnama={$pnama}&porder=N'>Kode</a></td>\n";
// echo "<td class=judul2 ><a href='listprodipt.php?tab={$tab}&pfld=".$pfld."&pfltr=".$pfltr."&pnama={$pnama}&porder=J'>Jenjang</a></td>\n";
// echo "<td class=judul2 ><a href='listprodipt.php?tab={$tab}&pfld=".$pfld."&pfltr=".$pfltr."&pnama={$pnama}&porder=U'>Nama</a></td>\n";
// echo "</tr></thead>\n";
$i = 0;
