                          ...
                          if ( $dt[JENIS]==3 ) { //2 = Per Semester
                            ...
                            if (sqlnumrows($hbeasiswa)>0) {
                              ...
                              #$dt[BATAS]=$dt[BATAS]*(100-$dbeasiswa[DISKON])/100;
								...
								#echo $dt[BATAS]."mmm".$dt2[TOTAL]."bbb".$dt[BATAS].'<br>'; 
                            #$lunas++;
                            if ($dt[BATAS]>0 && $dt2[TOTAL]<$dt[BATAS]) { // Tidak Lunas
                              #echo "sini";
							  $lunas--;
...
function set_status_login( $id, $jenis, $login = 1, $token = "", $tipe = "" )
{
    ...
    if ( $jenis == 3 )
    {
        $q = "UPDATE {$tabel} SET STATUSLOGIN2='{$login}',STATUSLOGIN='{$login}' {$timelogout} WHERE ID='{$id}'";
    }
    else
    {
...
function angkatoteks($a) {
	...
	if ($angka[$a]!="")	{
		...
		if ($a > 11 && $a < 20) {
			...
			$t1=floor($a / 100);
			$t2=$a % 100;
			echo "t2 kesini = ".$t2.'<br>';
			echo "t1 kesini = ".$t1.'<br>';
